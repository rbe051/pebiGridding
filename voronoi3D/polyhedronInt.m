...
for i = 1:nK
    ...
    faces = G.cells.faces(G.cells.facePos(cells(i)):G.cells.facePos(cells(i)+1)-1);  
    nodePos = arrayfun(@colon, G.faces.nodePos(faces), G.faces.nodePos(faces+1)-1, 'Uniform', false);
    nodes = cellfun(@(x) G.faces.nodes(x), nodePos, 'uniform', false);
    ...
    I(i,:) = vol*repmat(w,1,nTri).*rldecode(D,4*ones(nTri,1),1)'*f(Xhat,i);
